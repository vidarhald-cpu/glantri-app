...
| Campaign workspace | GM, player | `apps/web/src/lib/campaigns`, `apps/web/app/(app)/campaigns`, `apps/api/src/routes/scenarios.ts` | Campaign workspace/detail tests, `scenarios.test.ts`, scenario service integration tests, Playwright seeded campaign smoke | Player-side browser smoke for campaign access | High |
